...
mfrc522_status_t mfrc522_auth(uint8_t auth_mode, uint8_t block_addr,
		uint8_t *sector_key, uint8_t *ser_num) {
	mfrc522_status_t status;
	uint16_t recv_bits;
	uint8_t i;
	uint8_t buff[12];
	...
	if ((status != MI_OK)
			|| (!(mfrc522_read_register(MFRC522_REG_STATUS2) & 0x08))) {
		status = MI_ERR;
	}
...
mfrc522_status_t mfrc522_read(uint8_t block_addr, uint8_t *receive_data) {
	...
	status = mfrc522_to_card(PCD_TRANSCEIVE, receive_data, 4, receive_data,
			&unLen);
