...
/**
 * @brief Status enumeration for the MFRC522 module.
 */
typedef enum {
    MI_OK = 0,      /*!< Operation successful */
    MI_NOTAGERR,    /*!< No tag found */
    MI_ERR          /*!< Error occurred */
} mfrc522_status_t;

/**
 * @brief Structure to hold data related to the RC522 RFID module.
 */
typedef struct {
    uint8_t card_id[5];         /*!< ID of the RFID card */
    uint8_t flag_the;           /*!< Flag indicating specific condition */
    uint8_t byte_first;         /*!< First byte of data */
    uint8_t byte_second;        /*!< Second byte of data */
    uint8_t dem;                /*!< Counter or demarcation */
    uint8_t byte_data[1];       /*!< Array to store data bytes */
    uint8_t old_id[5];          /*!< Previous ID for comparison */
} rc522_data;
